...
#    print(" rel_err    =", mc_err_i500/vals[i500])
    ...
    dy_variants = [
        ("dy_ht_leadjet_rewght",       "-",  "DY w/ jet pT reweight"),
        ("dy_ht_lo_kfactor",                  "-", "LO DY"),
        ("dy_ht_lo_dijetmass_rewght", "-",  "DY w/ mjj reweight"),
    ]
    ...
    for (subdir, ls, lbl), col in zip(dy_variants, colors):
...
#    )
    ...
    bg_vals = tot.values(flow=False)
    # edges is already defined above as tot.axes[0].edges
    ...
    for (subdir, ls, lbl), col in zip(dy_variants, colors):
        ...
        # rebin exactly as above
        if "mass_fourobject" in hist_key:
            edges_custom = [0,800,1000,1200,1400,1600,2000,2400,2800,3200,8000]
            h = plotter.rebin_hist(raw_h, edges_custom)
        elif "pt_leading_jet" in hist_key:
            edges_custom = [0,40,100,200,400,600,800,1000,1500,2000]
            h = plotter.rebin_hist(raw_h, edges_custom)
        else:
            h = plotter.rebin_hist(raw_h)
        h = plotter.scale_hist(h)

        # compute new ratio = (Bkg + DY_variant) / Bkg
        new_vals = (tot + h).values(flow=False)
        ratio_line = new_vals / bg_vals
