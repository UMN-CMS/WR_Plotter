...
def plot_stack(plotter, region, variable):
    bkg_stack = [h for h in plotter.stack_list if hasattr(h, "axes")]
    bkg_labels = plotter.stack_labels
    bkg_colors = plotter.stack_colors

    # Only sum if we actually have histograms
    if not bkg_stack:
        logging.error("No background histograms to plot")
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, "No histograms found", ha="center", va="center")
        return fig

    # sum from the first element to avoid '0 + hist'
    tot = bkg_stack[0]
    for h in bkg_stack[1:]:
        tot = tot + h

    data_stack = [h for h in plotter.data_hist if hasattr(h, "axes")]
    data = None
    if data_stack:
        data = data_stack[0]
        for h in data_stack[1:]:
            data = data + h
